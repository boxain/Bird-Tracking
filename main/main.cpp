    ...
    if(event_base == WIFI_EVENT) {
        ...
        switch (event_id){
            case WIFI_EVENT_STA_START: {
                ...
                ESP_LOGI("WIFI Event", "Wifi station connect to the AP fail.\n");
...
void wifi_init_task(){
    ...
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();  // use wifi defult init config
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI("WIFI_INIT", "Connected to SSID:%s, password:%s", CONFIG_ESP_WIFI_SSID, CONFIG_ESP_WIFI_PASSWORD);
        char* access_token = auth_http_request();
        if(access_token){
            websocket_init_task(access_token);
        }
    }else if(bits & WIFI_FAIL_BIT){
        ESP_LOGI("WIFI_INIT", "Failed to connect to SSID:%s, password:%s", CONFIG_ESP_WIFI_SSID, CONFIG_ESP_WIFI_PASSWORD);
    }
}
